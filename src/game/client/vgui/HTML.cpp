...
{
#ifdef FLOP_DLL
	const char* msg_t = pCmd->pchMessage;
	if (!strcmp(msg_t, "action_serverbrowser"))
	{
		engine->ClientCmd_Unrestricted("gamemenucommand OpenServerBrowser");
		if (m_SteamAPIContext.SteamHTMLSurface())
			m_SteamAPIContext.SteamHTMLSurface()->JSDialogResponse(m_unBrowserHandle, true);
		RequestFocus();
		return;
	}

	if (!strcmp(msg_t, "action_servercreator"))
	{
	...
	if (!strcmp(msg_t, "action_settings"))
	{
...
#ifdef FLOP_DLL
	MessageBox* pDlg = new MessageBox("NOTIFICATION", (const char*)pCmd->pchMessage, this);
#else
...
{
#ifdef FLOP_DLL
	QueryBox* pDlg = new QueryBox("CONFIRMATION", (const char*)pCmd->pchMessage, this);
#else
